...
1. **API Layer** (Blue)
   - FastAPI application
   - Request/Response models for chat and deck persistence
   - Entry point for all user interactions
   - RESTful endpoints for CRUD operations on decks

2. **FSM Layer** (Orange)
   - FSM Orchestrator coordinates the workflow
   - State nodes implement the Draft-Verify-Refine loop
   - StateData tracks progress across iterations
   - Supports both new deck creation and deck modification flows

3. **Services Layer** (Green)
   - AgentDeckBuilderService: LLM-powered deck building with tool calling
   - LLMService: Structured deck analysis using Pydantic AI
   - QualityVerifierService: Heuristic + LLM quality analysis
   - VectorService: Semantic search via ChromaDB embeddings
   - PromptBuilder: Dynamic format-specific prompt generation

4. **Data Access Layer** (Purple)
   - CardRepository: CAG pattern (Cache-Augmented Generation) for cards
   - DeckRepository: CRUD operations for deck persistence
   - DatabaseService: SQLite persistence with indexes for both cards and decks

5. **Caching Layer** (Pink)
   - ICache interface for strategy pattern
   - LRUCache implementation
   - CacheStats for performance monitoring

6. **Models Layer** (Yellow)
   - Core domain models (MTGCard, Deck, DeckCard)
...
1. **Finite State Machine**: Draft → Verify → Refine loop with iteration tracking
2. **Cache-Augmented Generation (CAG)**: Two-tier lookup (cache → database)
3. **Strategy Pattern**: ICache interface with pluggable implementations
4. **Repository Pattern**: CardRepository abstracts data access
5. **Agent Pattern**: LLM agents with tool-calling for intelligent decisions
6. **Builder Pattern**: PromptBuilder for dynamic prompt construction

### Data Flow

**New Deck Creation:**
```
User Request → API → FSMOrchestrator → ParseRequest → BuildInitialDeck
→ VerifyQuality → (if quality < threshold) → RefineDeck → VerifyQuality
→ (repeat until quality threshold met or max iterations) → Return Deck
```

**Deck Modification:**
```
User Request + Existing Deck → API → FSMOrchestrator → UserModificationNode
→ AgentDeckBuilder (refine_deck) → Modified Deck → Return Result
```

**Card Search:**
```
Search Request → CardRepository → (Check Cache) → (If miss) → DatabaseService
→ (Promote to Cache) → Return Cards
```

**Semantic Search:**
```
Query → VectorService → ChromaDB → Candidate IDs → CardRepository
→ Full Card Data → Apply Filters → Return Filtered Results
```

**Deck Persistence (Save):**
```
Frontend Save Request → POST /api/decks → DeckRepository.save_deck()
→ DatabaseService (INSERT into decks table) → Return deck_id → Frontend
```

**Deck Persistence (Load):**
```
Frontend Load Request → GET /api/decks/{id} → DeckRepository.get_deck_by_id()
→ DatabaseService (SELECT from decks table) → Return Deck + Metadata → Frontend
```

**Deck Persistence (Update):**
```
Frontend Update Request → PUT /api/decks/{id} → DeckRepository.update_deck()
→ DatabaseService (UPDATE decks table) → Return success → Frontend
```

**Deck Persistence (List):**
```
Frontend List Request → GET /api/decks?format=Standard → DeckRepository.list_decks()
→ DatabaseService (SELECT with filters) → Return deck list → Frontend
```
